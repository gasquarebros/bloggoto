...
<table class="table " style="text-align: left;">
	<thead class="first">
		<tr>
			...
			<th><?=get_label('pro_cate_sequence').add_sort_by('pro_cate_sequence',$module);?></th>
 ...
 <?php
	if (! empty ( $records )) {
		foreach ( $records as $val ) { ?>
			<tr class="<?php echo $val['pro_cate_primary_id']."_maintr" ?>" >
				<td>
					<div class="checkbox3 checkbox-inline checkbox-check checkbox-light"><?php echo form_checkbox('id[]',$val['pro_cate_primary_id'],'',' class="multi_check" type="checkbox" id="'.$val['pro_cate_primary_id'].'"   ');?>
						<label for="<?php echo $val['pro_cate_primary_id'];?>" class="chk_box_label"></label>
					</div>
				</td>
				<td class="top_input"> <span class="spn"><?php echo output_value($val['pro_cate_name']);?></span></td>
	
				<td class="top_input"> <span class="spn"><?php echo output_value($val['pro_cate_sequence']);?></span> </td>
	
	
				<td class="top_input" ><a href="javascript:;"><?php echo show_status($val['pro_cate_status'],$val['pro_cate_primary_id']);?></a></td>
				<td>
					<a href="<?php echo admin_url().$module.'/edit/'.encode_value($val['pro_cate_primary_id']);?>"><i class="fa fa-edit" title="<?php echo get_label('edit')?>"></i></a>&nbsp;
					
					<a href="javascript:;" class="delete_record" id="<?php echo encode_value($val['pro_cate_primary_id']);?>" data="Delete"><i class="fa fa-trash" title="<?php echo get_label('delete')?>"></i></a>
				</td>
			</tr>
<?php  } } else { ?>
<tr class="no_records">
