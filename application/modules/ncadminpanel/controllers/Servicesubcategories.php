...
class Servicesubcategories extends CI_Controller {
	public function __construct() {
		...
		$this->folder = "servicesubcategories/";
		$this->table = "service_subcategories";
		$this->category_table = "service_categories";
	...
	public function index() {
		...
		$category_array = get_service_category('','','class="form-control" id="product_category"');		
		...
		if ($this->input->post ( 'action' ) == "Add") {
			...
			if ($this->form_validation->run () == TRUE) {
				...
				if (isset ( $_FILES ['pro_subcate_image'] ['name'] ) && $_FILES ['pro_subcate_image'] ['name'] != "") {
					$pro_cate_image = $this->common->upload_image ( 'pro_subcate_image', get_company_folder() . "/".$this->lang->line('service_subcategory_image_folder_name') );
				}
				
				$category = $this->Mydb->get_record('*',$this->category_table,array('ser_cate_id'=>post_value('product_category')));
				$insert_array = array (
						'pro_subcate_name' => post_value ( 'pro_subcate_name' ),
						'pro_subcate_id' => $pro_subcate_id,
						'pro_subcate_category_primary_id' => $category['ser_cate_primary_id'],
                        'pro_subcate_category_id'   => $category['ser_cate_id'],
	...
	public function edit($edit_id = NULL) {
		...
		if ($this->input->post ( 'action' ) == "edit") {
			...
			if ($this->form_validation->run () == TRUE) {
				...
				$image_arr = array();
				if (isset ( $_FILES ['pro_subcate_image'] ['name'] ) && $_FILES ['pro_subcate_image'] ['name'] != "") {
					$upload_image = $this->common->upload_image ( 'pro_subcate_image', get_company_folder() . "/".$this->lang->line('service_subcategory_image_folder_name') );
					$image_arr = array (
							'pro_subcate_image' => $upload_image
					);
						
					$this->common->unlink_image($record['pro_subcate_image'],get_company_folder(),$this->lang->line('service_subcategory_image_folder_name')); /* unlink image..  */
				}
				elseif($this->input->post('remove_image')=="Yes") {
					...
					$this->common->unlink_image($record['pro_subcate_image'],get_company_folder(),$this->lang->line('service_subcategory_image_folder_name'));
				}
	...
	function action() {
		...
		if ($postaction == 'Delete' && ! empty ( $ids )) {
			...
			{
				/*  unlink  images */
				$this->common->delete_unlink_image($this->lang->line('service_subcategory_image_folder_name'), "pro_subcate_image",$this->primary_key ,$this->table,$ids);
	...
	public function maincategory_exists() {
		...
		$where = array (
				'ser_cate_id' => trim ( $name )
		);
		$result = $this->Mydb->get_record ( 'ser_cate_primary_id', $this->category_table, $where );
		if (empty ( $result ) ) {
