...
class Services extends CI_Controller 
{
	...
	public function __construct() 
	{
		...
		$this->authentication->admin_authentication ( get_label ( 'services_module' ) );
		$this->module_label = get_label ( 'services_label' );
		...
		if ($this->input->post ( 'action' ) == "Add") {
			...
			if ($this->form_validation->run () == TRUE) {
				...
				$insert_array = array (
					...
					'ser_customer_id' => post_value ( 'product_customer_id' ),
	...
	public function productnameexists() {
		$name = $this->input->post ( 'ser_title' );
	...
	public function get_subcategory() {
		check_ajax_request (); /* skip direct access */
		$category_id = post_value ( 'service_category' );

		$where = array();
		if($category_id !=''){
			$where = array('pro_subcate_category_primary_id' => $category_id);
		}
		$dropdown = get_service_subcategory ( $where, '', 'class="form-control" required id="ser_subcategory" ' );	
		echo json_encode ( array (
				'html' => $dropdown 
		) );
	}
