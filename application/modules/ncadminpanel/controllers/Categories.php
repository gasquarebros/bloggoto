...
class Categories extends CI_Controller {
	...
	function ajax_pagination($page = 0) {
		...
		$select_array = array ('pro_cate_primary_id ','pro_cate_name','pro_cate_status','pro_cate_sequence');				
	...
	public function add() {
		...
		if ($this->input->post ( 'action' ) == "Add") {
			...
			$this->form_validation->set_rules ( 'pro_cate_name', 'lang:pro_cate_name', 'required|callback_category_exists' );
			$this->form_validation->set_rules ( 'status', 'lang:status', 'required' );
			$this->form_validation->set_rules ( 'pro_cate_image', 'lang:pro_cate_image', 'callback_validate_image' );

			if ($this->form_validation->run () == TRUE) {
				$pro_cate_id = get_guid($this->table,'pro_cate_id');
				$pro_cate_slug  = make_slug($this->input->post('pro_cate_name'),$this->table,'pro_cate_slug');
			 	$pro_cate_sequence = ((int)$this->input->post('pro_cate_sequence') == 0)?  get_sequence('pro_cate_sequence',$this->table) : $this->input->post('pro_cate_sequence'); 
				
				/* upload image */
				$pro_cate_image = "";
				if (isset ( $_FILES ['pro_cate_image'] ['name'] ) && $_FILES ['pro_cate_image'] ['name'] != "") {
					$pro_cate_image = $this->common->upload_image ( 'pro_cate_image', get_company_folder() . "/".$this->lang->line('product_category_image_folder_name') );
				}
				
				
				$insert_array = array (
						'pro_cate_name' => post_value ( 'pro_cate_name' ),
						'pro_cate_id' => $pro_cate_id,
						'pro_cate_slug'=>$pro_cate_slug,
						'pro_cate_short_description' => post_value ( 'pro_cate_short_desc' ),
						'pro_cate_description' => post_value ( 'pro_cate_desc' ),
						'pro_cate_image' => $pro_cate_image,
						'pro_cate_status' => (post_value('status')=="A" ? 'A' : 'I'),
						'pro_cate_sequence' => $pro_cate_sequence,
						'pro_cate_created_on' => current_date (),
						'pro_cate_created_by' => get_admin_id (),
	...
	public function edit($edit_id = NULL) {
		...
		if ($this->input->post ( 'action' ) == "edit") {
			...
			if ($this->form_validation->run () == TRUE) {
				...
				$pro_cate_slug  = make_slug($this->input->post('pro_cate_name'),$this->table,'pro_cate_slug',array($this->primary_key."!=" =>$record [$this->primary_key]));
				$pro_cate_sequence = ((int)$this->input->post('pro_cate_sequence') == 0)?  get_sequence('pro_cate_sequence',$this->table) : $this->input->post('pro_cate_sequence');
				...
				$update_array = array (
						...
						'pro_cate_updated_by' => get_admin_id (),
				...
				if (isset ( $_FILES ['pro_cate_image'] ['name'] ) && $_FILES ['pro_cate_image'] ['name'] != "") {
					...
					$this->common->unlink_image($record['pro_cate_image'],get_company_folder(),$this->lang->line('product_category_image_folder_name')); /* unlink image..  */
				}
		...
		if ($postaction == 'Sequence' && ! empty ( $ids )) {
			...
			if(!empty($ids))
			{
				foreach($ids as $primary_id )
				{
					...
					{
						$update_values = array(
								"pro_cate_sequence" => $sequnce_value,
								"pro_cate_updated_on" => current_date (),
								'pro_cate_updated_by' => get_admin_id (),
								'pro_cate_updated_ip' => get_ip ()	);
	...
	public function category_exists() {
		$name = $this->input->post ( 'pro_cate_name' );
		...
		if ($edit_id != "") {
			$where = array_merge ( $where, array (
					"pro_cate_primary_id !=" => $edit_id
			) );
				
		}
		$result = $this->Mydb->get_record ( 'pro_cate_primary_id', $this->table, $where );
		if (! empty ( $result ) ) {
