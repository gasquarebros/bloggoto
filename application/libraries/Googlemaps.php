...
class Googlemaps {
	...
	var $apiKey						= 'AIzaSyBSZ4vRWx7D0YloUNNSp8TRRXiuqlWi5dE'; 						// If you've got an API key you can use it by passing this parameter. Setup an API key here: https://code.google.com/apis/console
	...
	var $onclickfunction = 'setSelection';						// The JavaScript action to perform when a place is selected
	var $ondeletefunction	= 'deleteSelectedShape';			// The JavaScript action to perform when a place is selected
	...
	function add_polygon($params = array())
	{
		...
		$polygon['editable'] = 'TRUE';							// Defines if the polygon is clickable
		$polygon['draggable'] = 'TRUE';							// Defines if the polygon is clickable
		...
		if ($polygon['editable']!="") { 
			$polygon_output .= '
					google.maps.event.addListener(polygon_'.count($this->polygons).'.getPath(), "set_at", function() {
						var region_points=polygon_'.count($this->polygons).'.getPath().getArray();
						var region_type="polygon";
						document.getElementById("region_type'.$this->map_name.'").value=region_type;
						document.getElementById("region_points'.$this->map_name.'").value=region_points;
					});
					google.maps.event.addListener(polygon_'.count($this->polygons).'.getPath(), "insert_at",function() {
						var region_points=polygon_'.count($this->polygons).'.getPath().getArray();
						var region_type="polygon";
						document.getElementById("region_type'.$this->map_name.'").value=region_type;
						document.getElementById("region_points'.$this->map_name.'").value=region_points;
					});
					google.maps.event.addListener(polygon_'.count($this->polygons).'.getPath(), "remove_at",function() {
						var region_points=polygon_'.count($this->polygons).'.getPath().getArray();
						var region_type="polygon";
						document.getElementById("region_type'.$this->map_name.'").value=region_type;
						document.getElementById("region_points'.$this->map_name.'").value=region_points;
	...
	function add_circle($params = array())
	{
		...
		$circle['editable'] = 'TRUE';							// Defines if the circle is clickable
		$circle['draggable'] = 'TRUE';							// Defines if the circle is clickable
		...
		if ($circle['radius']>0 && $circle['center']!="") {
			...
			if ($circle['editable']!="") { 
				$circle_output .= '
				google.maps.event.addListener(circle_'.count($this->circles).', "bounds_changed", function() {
					var region_points=circle_'.count($this->circles).'.getCenter().lat().toString();
					region_points+='.'"|"+'.'circle_'.count($this->circles).'.getCenter().lng().toString();
					var region_radius=circle_'.count($this->circles).'.getRadius();
					var region_type="circle";
					document.getElementById("region_radius'.$this->map_name.'").value=region_radius;
					document.getElementById("region_type'.$this->map_name.'").value=region_type;
					document.getElementById("region_points'.$this->map_name.'").value=region_points;
				});
				';
	...
	function add_rectangle($params = array())
	{
		...
		$rectangle['editable'] = 'TRUE';							// Defines if the rectangle is clickable
		$rectangle['draggable'] = 'TRUE';							// Defines if the rectangle is clickable
		...
		if ($rectangle['positionSW']!="" && $rectangle['positionNE']!="") {
			...
			if ($rectangle['editable']!="") { 
				...
				$rectangle_output .= '
					...
					google.maps.event.addListener(rectangle_'.count($this->rectangles).', "bounds_changed", function() {
					console.log("chnage");
						
						console.log("asdsad");
						/*function showNewRect(event) {*/
						console.log('.count($this->rectangles).');
							var ne = rectangle_'.count($this->rectangles).'.getBounds().getNorthEast();
							var sw = rectangle_'.count($this->rectangles).'.getBounds().getSouthWest();
							var region_points=rectangle_'.count($this->rectangles).'.getBounds().toJSON().south.toString();
							region_points+='.'","+'.'rectangle_'.count($this->rectangles).'.getBounds().toJSON().north.toString();
							region_points+='.'"|"+'.'rectangle_'.count($this->rectangles).'.getBounds().toJSON().west.toString();
							region_points+='.'","+'.'rectangle_'.count($this->rectangles).'.getBounds().toJSON().east.toString();
							var region_type="rectangle";
							document.getElementById("region_type'.$this->map_name.'").value=region_type;
							document.getElementById("region_points'.$this->map_name.'").value=region_points;
						/*}*/
					});
				
					/*rectangle_'.count($this->rectangles).'.addListener("bounds_changed", showNewRect('.count($this->rectangles).'));*/
				';
			}
	...
	function create_map()
	{
		...
		$this->output_js_contents .= "var contentString = ''; 
		
		function setSelection".$this->map_name."(shape) {
            ...
            selectedShape = shape;
            console.log(selectedShape.map);
            EditcontentString = '';
            try {
                if (shape.getCenter()) {
                    var newShape = shape.overlay;
                    var pathShape = shape.getCenter();
                ...
                if (shape.bounds) {
					var newShape = shape.overlay;
                    var pathShape = shape.getBounds();
				}
                else if(shape.getPath()) {
					var pathShape = shape.getPath();
					for (var i = 0; i < pathShape.getLength() ; i++) {
						var xy = pathShape.getAt(i);
						if (EditcontentString.length > 0) EditcontentString += ',';
						EditcontentString += i + ':' + xy.lat() + ':' + xy.lng();
					}
				}
            }
            if (EditcontentString == contentString) {
                var txtName = 'btnCreate';
          }
          else {
              var txtName = 'btnCreate';
          }
      ...
      function deleteSelectedShape(mapid,mapname) {
          if (selectedShape) {
          console.log(mapid);
          console.log(selectedShape);
          console.log('entering into this');
          console.log(selectedShape.map.__gm.Ia.id);
          console.log(mapid);
			/*if(selectedShape.map.__gm.Ia.id == mapid) {*/
				  try {
					  var pathShape = selectedShape.getPath();
			   
					  if (confirm('Are you sure want to remove this?') == true) {
							selectedShape.setMap(null);
							document.getElementById('region_type'+mapname).value='';
							document.getElementById('region_points'+mapname).value='';
							document.getElementById('region_radius'+mapname).value='';
							var dpacket = EditcontentString;
      ...
      ";
		...
		if ($this->drawing) {
			...
			google.maps.event.addListener(drawingManager, "overlaycomplete", function(event) {
				...
				if(event.type == "circle" || "rectangle" == event.type)
				{
					if(event.type == "circle") 
					{
						var region_points=event.overlay.getCenter().lat().toString();
						region_points+='.'"|"+'.'event.overlay.getCenter().lng().toString();
						var region_radius=event.overlay.getRadius();
						var region_type="circle";
						document.getElementById("region_radius'.$this->map_name.'").value=region_radius;
						document.getElementById("region_type'.$this->map_name.'").value=region_type;
						document.getElementById("region_points'.$this->map_name.'").value=region_points;
						...
						var region_points=event.overlay.getBounds().toJSON().south.toString();
						region_points+='.'","+'.'event.overlay.getBounds().toJSON().north.toString();
						region_points+='.'"|"+'.'event.overlay.getBounds().toJSON().west.toString();
						region_points+='.'","+'.'event.overlay.getBounds().toJSON().east.toString();
						var region_type="rectangle";
						document.getElementById("region_type'.$this->map_name.'").value=region_type;
						document.getElementById("region_points'.$this->map_name.'").value=region_points;
					}
				...
				{
					var region_type="marker";
					var region_points=event.overlay.getPosition().toJSON().lat.toString();
					region_points+='.'"|"+'.'event.overlay.getPosition().toJSON().lng.toString();
					document.getElementById("region_type'.$this->map_name.'").value=region_type;
					document.getElementById("region_points'.$this->map_name.'").value=region_points;
				}
				else if("polygon" == event.type) {
					var region_points=event.overlay.getPath().getArray();
					var region_type="polygon";
					document.getElementById("region_type'.$this->map_name.'").value=region_type;
					document.getElementById("region_points'.$this->map_name.'").value=region_points;
				}
				...
				google.maps.event.addListener(newShape, "click", function () {
					'.$this->onclickfunction.$this->map_name.'(newShape);
				});
				'.$this->onclickfunction.$this->map_name.'(newShape);
			...
			if (count($this->drawingOnEdit)) {
				...
				if (newShape.type==google.maps.drawing.OverlayType.RECTANGLE) {
					google.maps.event.addListener(newShape, "bounds_changed", function(event) {
						console.info(event.overlay.getBounds().toJSON());
		...
		$this->output_js_contents .='
		  ...
		  function setMapOnAll_'.$this->map_name.'(map) {
			for (var i = 0; i < markers_'.$this->map_name.'.length; i++) {
			  markers_'.$this->map_name.'[i].setMap(map);
			}
		  }
	
		  
		  function clearMarkers_'.$this->map_name.'() {
			setMapOnAll_'.$this->map_name.'('.$this->map_name.');
		  }


		  function deleteMarkers_'.$this->map_name.'('.$this->map_name.') {
			clearMarkers_'.$this->map_name.'();
		  }';
