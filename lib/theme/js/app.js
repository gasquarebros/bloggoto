		...
		$("#common_form").ajaxSubmit({
			...
			success : function(data) {
				...
				if (response.status == "success") {
					...
					$('#common_form').find(".alert_msg,.container_div").show();
					$('#common_form').find(".alert_msg").html(data.message);
...
alertheader =  (typeof(alertheader) =="undefined" )?  "Confirmation" :  alertheader;
	$('body').append('<div class="alrt_overlay"></div><div class="custom_alert"><div class="custom_alertin"><div class="alert_height"><div class="alert_header">'+ alertheader +'</div><div class="alert_body"> '+ alrtmessage +' <div class="alt_btns"><a href="javascript:;" class="btn btn-primary confirm" id="alt1">Yes</a><a href="javascript:;" class="btn btn-info reject" id="alt0">No</a></div></div></div></div></div>');
